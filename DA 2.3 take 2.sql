WITH FilteredContacts AS (
    ...
    WHERE 
		...
		AND 
			(
				(
					[OR] = (select min(OR) FROM CON1 AND [SQ] = 1
					AND 
						(
						TRIM(LW) = 'Y'
						OR TRIM(ERH) = 'Y'
			...
			) -- students 512761 and 512762 have their grandma listed with portal access, but she should not be included because the first PG is listed as P1
),
TopContacts AS (
...
WITH FilteredContacts AS (
    ...
    SELECT 
		...
		CD [contact type],
		NP [notification preference],  -- 0 means do not contact
        ROW_NUMBER() OVER (PARTITION BY CON.PID ORDER BY CON.SQ) AS ContactRank
    FROM 
        CON
    WHERE 
		DEL = 0
